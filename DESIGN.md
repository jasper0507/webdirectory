...
阅读顺序：页头（圆印 + 字标 / 金点 + 印戳）→ 眉题 → 两字碑 → 两行耳语 → 提问线 → 键位提示 → 题名索引（输入后）→ 七词 → 页脚。提问时碑、眉题、耳语和提示留在原位，七词收起；厅加上 `is-asking`，上内边距收到区块尺度，碑缩小但仍在原位，把视口让给索引。
...
- `/` 聚焦提问；Logo 回厅清空提问、收起索引、不保留焦点。Escape 先取消选中，再清空提问。选中题名，或无选中且唯一题名时，回车打开 URL。选中标签或点七词则带该标签进货架。空提交、多条命中未选中，仍进货架。选中行左侧金点，题名保持墨或灰，不整行改成金字。
...
- 卡片：白纸面落在档案纸上。名称与 URL 必显；空描述不占位。点卡片打开外链。点卡片上的标签 = 追加精确约束。标签字级保持印戳，热区外扩，不靠 `min-height: 44px` 把卡片撑高。
...
厅是一列居中的题签：页头 sticky，主列把碑送到视口中部（移动上内边距 5.5rem，`48rem` 起 14.3125rem）。提问态或视口高度低于 `48rem`（含手机横屏）时收上内边距，碑留在原位。厅内不为卡片网格纵滚。货架容器 `min(72rem, calc(100% - 2rem))`；网格移动 1 列，`768px` 起 2 列，`1024px` 起 3 列，间隙 1rem。页头/货架顶栏水平内边距 1.25rem，`48rem` 起 3.5rem，并吃进 `safe-area-inset`。节奏只用少数档：控件间隙 0.35–0.75rem，区块 1.25–2.5rem。
...
页脚文字钮，11px、字距 0.16em。白日灰墨，夜间用墨，与题签分开。金留给悬停/焦点。不放在 Logo 上。
