...
- `/` 聚焦提问；Logo 回厅清空提问、收起索引、不保留焦点。选中题名，或无选中且唯一题名时，回车打开 URL。选中标签或点七词则带该标签进货架。空提交、多条命中未选中，仍进货架。选中行变金。
...
输入后出现，碑同时让位。匹配标签最多 3 条，题名补齐到共约 7 行。无卡片、无阴影。悬停或选中变金。无匹配时留一句耳语，不把七词整组收走。满编索引不得把七词或页脚顶出视口。

### Constraint chips

提问词与已选标签，行首印「已约束」。金色叉是可见移除标记，整粒芯片可点，读屏名为「移除提问 … / 移除标签 …」。共现标签行首印「还可加上」，样式与七词相同，只统计当前命中且不含已选。
