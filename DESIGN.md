...
厅是一列居中的题签：页头 sticky，主列把碑送到视口中部（移动上内边距 5.5rem，`48rem` 起 14.3125rem）。视口高度低于 `48rem`（含手机横屏）时收上内边距，碑尺度不因提问改变。厅内不为卡片网格纵滚。货架容器 `min(72rem, calc(100% - 2rem))`；网格移动 1 列，`768px` 起 2 列，`1024px` 起 3 列，间隙 1rem。页头/货架顶栏水平内边距 1.25rem，`48rem` 起 3.5rem，并吃进 `safe-area-inset`。节奏只用少数档：控件间隙 0.35–0.75rem，区块 1.25–2.5rem。
