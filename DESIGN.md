...
- 网格：移动 1 列，`768px` 起 2 列，`1024px` 起 3 列。卡片圆角不超过 2px，发丝边，无硬偏移阴影、无彩条。卡片按统一解剖对齐，不把网格撑满视口；页脚仍沉底。
...
圆印 + 衬线字标。点击回到搜索厅，不切换主题。印 1.75rem，档案纸底、金环描边，角标是 Lucide `library` 的四线书架（墨色描边）。悬停印放大到 1.05。白日夜间同一构图，填色跟随 `data-paper`。
...
`--color-sheet` 落在档案纸上。圆角 2px，发丝边。悬停边改金。名称 serif。题名一行、描述两行、网址一行裁切；空描述仍占两行。卡片上的标签在链接外，点按进入该类；悬停金线，读屏名为「进入标签 …」。当前类按下态金线、`aria-current`。卡片主链接读屏名为「打开 …（新标签页）」。
