...
空厅阅读顺序：页头（圆印 + 字标 / 金点 + 印戳）→ 眉题 → 两字碑 → 两行耳语 → 提问线 → 键位提示 → 七词 → 页脚。

提问态阅读顺序：页头 → 提问线 → 题名索引（或无匹配耳语）→ 七词 → 页脚。碑、眉题、耳语和键位让位；字标仍在页头。

- 空厅桌面一屏题签；提问后仍一屏，厅内不为卡片网格纵滚。索引最多约七行，无卡片、无阴影，锁在提问线与页脚之间。
- 提问是底边线，不是输入盒。空厅不自动聚焦；`/`、「跳到提问」或点提问线才聚焦，金线此时从左生长。
- 七词按标签引用次数动态取最多七个；无填充、无描边。有提问时降对比，不整组消失。
- `/` 聚焦提问；Logo 回厅清空提问、收起索引、不保留焦点。选中题名，或无选中且唯一题名时，回车打开 URL。选中标签或点七词则带该标签进货架。空提交、多条命中未选中，仍进货架。选中行左侧金点，题名保持墨或灰，不整行改成金字。
- 空提交进入全部货架，不再只闪金线。厅内无匹配时索引位留一句可回车进货架的耳语。
...
厅是一列居中的题签：页头 sticky，空厅主列把碑送到视口中部（移动上内边距 5.5rem，`48rem` 起 14.3125rem）。提问态（`.hall.is-asking`）收起碑区、撤掉中部顶垫（移动 1.25rem，`48rem` 起 2.5rem），索引吃提问线与七词之间的剩余高度；七词与页脚留在视口内。厅内不为卡片网格纵滚。货架容器 `min(72rem, calc(100% - 2rem))`；网格移动 1 列，`768px` 起 2 列，`1024px` 起 3 列，间隙 1rem。页头/货架顶栏水平内边距 1.25rem，`48rem` 起 3.5rem。节奏只用少数档：控件间隙 0.35–0.75rem，区块 1.25–2.5rem。
...
圆印 + 衬线字标。点击回到搜索厅，不切换主题。印 1.75rem，档案纸底、墨角标、金环描边。悬停印放大到 1.05。白日夜间同一张印。

### Search line

底边 1px 发丝，serif 18px / 28px。厅内聚焦时 2px 金线从左生长（0.6s ease-royal）。货架同一行、通栏，聚焦只把底边改金。提交钮是 2.75rem 金描边圆，检索图标用墨。
...
输入后出现，碑同时让位。匹配标签最多 3 条，题名补齐到共约 7 行。无卡片、无阴影。悬停底边金线，选中左侧金点；题名保持灰或墨，不整行改成金字。无匹配时留一句耳语，不把七词整组收走。满编索引不得把七词或页脚顶出视口。

### Constraint chips

提问词与已选标签，行首印「已约束」。移除标记是金线叉，标签保持墨色；整粒芯片可点，读屏名为「移除提问 … / 移除标签 …」。共现标签行首印「还可加上」，样式与七词相同，只统计当前命中且不含已选。

### Bookmark cards

`--color-sheet` 落在档案纸上。圆角 2px，发丝边。悬停边改金。名称 serif；空描述不占位。卡片上的标签在链接外，追加精确约束；悬停金线，读屏名为「用标签 … 约束货架」。
