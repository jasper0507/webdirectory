...
同一套货架。无约束 = 全部目录；有提问词或标签约束 = 命中集合。结果顶用印戳列出本次结果相关标签（名称 · 条数）；点标签切换精确约束，卡片不再按首个标签插入分块标题。无约束时卡片仍按首个标签块的顺序排列。

- 顶栏：Logo 回厅、提问行、当前命中条数。顶栏与提问行叠成一块吸顶，改提问不必先滚回页头。货架顶栏与提问行、卡片对齐，不再套厅面的 3.5rem 左右内边距。吸顶块不另画底部分隔，提问线就是下沿。提问词留在提问框里改；标签约束在结果顶印戳上点按切换。
- 提问行与厅共用通栏底边和圆钮，聚焦时底边改金；不使用生长金线。题线停在圆钮之前，不穿过金环。
- 卡片：白纸面落在档案纸上。名称与 URL 必显。题名一行、描述两行、网址一行，过长裁切；空描述仍占两行，使同行卡片同高。点卡片打开外链。点卡片上的标签或结果顶标签 = 切换精确约束。标签字级保持印戳，未约束时待命短金线，热区外扩到 44px，不靠 `min-height: 44px` 把卡片撑高。约 2000 条时卡片分批上屏。
...
货架结果顶列出本次命中相关标签，印戳语气，名称后跟条数。点按切换精确约束；已约束项金线按下、`aria-pressed`，再点即解除。读屏名为「用标签 … 约束货架」或「移除标签 … 约束」。不另设「已约束」行。

### Bookmark cards

`--color-sheet` 落在档案纸上。圆角 2px，发丝边。悬停边改金。名称 serif。题名一行、描述两行、网址一行裁切；空描述仍占两行。卡片上的标签在链接外，点按切换精确约束；悬停金线，读屏名为「用标签 … 约束货架」或「移除标签 … 约束」。已约束的标签按下态金线、`aria-pressed`。卡片主链接读屏名为「打开 …（新标签页）」。
