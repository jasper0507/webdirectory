...
- 卡片：白纸面落在档案纸上。名称与 URL 必显；空描述不占位。点卡片打开外链。点卡片上的标签或结果顶标签 = 切换精确约束。标签字级保持印戳，未约束时待命短金线，热区外扩到 44px，不靠 `min-height: 44px` 把卡片撑高。约 2000 条时卡片分批上屏。
...
`--color-sheet` 落在档案纸上。圆角 2px，发丝边。悬停边改金。名称 serif；空描述不占位。卡片上的标签在链接外，点按切换精确约束；悬停金线，读屏名为「用标签 … 约束货架」或「移除标签 … 约束」。已约束的标签按下态金线、`aria-pressed`。卡片主链接读屏名为「打开 …（新标签页）」。
